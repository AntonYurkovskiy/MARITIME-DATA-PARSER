...
def remove_date_from_text(text: str, match: Optional[re.Match]) -> str:
    ...
    # нормализуем пробелы [web:149]
    remaining_text = ' '.join(remaining_text.split())
    return clean_letters_commas(remaining_text)


def extract_date_to_iso(text: Optional[str]) -> Optional[Tuple[str, str]]:
    """
    ...
    """
    # Пустая строка или None → None (как требуют тесты empty/None) [web:147]
    ...
    # Дата по паттерну есть, но невалидна → None (по тестам invalid_date)
    if iso_date is None:
        return None
